		...
		for ( itr2 = itr; itr2 != colliders.end(); itr2 ++ )
		{
			if ( itr != itr2 ) // to prevent an object checking with itself
			{
				if ( (*itr)->GetGameObject()->GetTag() == "Bullet"   || 
					 (*itr2)->GetGameObject()->GetTag() == "Bullet"  || 
					 (*itr)->GetGameObject()->GetTag() == "Missile"  || 
					 (*itr2)->GetGameObject()->GetTag() == "Missile" || 
					 (*itr)->GetGameObject()->GetTag() == "Powerup"  || 
					 (*itr2)->GetGameObject()->GetTag() == "Powerup" ||
					 (*itr)->GetGameObject()->GetTag() == "Asteroid" || 
					 (*itr2)->GetGameObject()->GetTag() == "Asteroid"||
					 (*itr)->GetGameObject()->GetTag() == "Player"   || 
					 (*itr2)->GetGameObject()->GetTag() == "Player"  
				   )
				{
