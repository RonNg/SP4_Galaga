...
void myApplication::MouseMove (int x, int y) {
	mouseInfo.lastX = x - HEIGHT_RESOLUTION/2;
	mouseInfo.lastY = WIDTH_RESOLUTION/2 - y;
...
{
	// Set camera position
	theCamera = new idCamera( idCamera::LAND_CAM );
...
void myApplication::TestRender ()
{
	...
	glPushMatrix();
		...
		glBegin(GL_QUADS);
			glTexCoord2f( 0, 1 ); glVertex2f( -dimensions.x, dimensions.y);
			glTexCoord2f( 0, 0 ); glVertex2f( -dimensions.x, -dimensions.y);
			glTexCoord2f( 1, 0 ); glVertex2f( dimensions.x, -dimensions.y );
			glTexCoord2f( 1, 1 ); glVertex2f( dimensions.x, dimensions.y );
		glEnd();
	glPopMatrix();
